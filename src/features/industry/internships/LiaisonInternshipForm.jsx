...
const schema = yup.object({
  ...
  duration: yup.string(),
  stipend: yup.number().min(0, 'Stipend must be positive'),
  skills: yup.array(),
  ...
  const fetchInternship = async () => {
    try {
      ...
      Object.keys(data).forEach(key => {
        if (key === 'skills') {
          const skillsArray = data.skills ? data.skills.split(', ').map(skill => ({ value: skill, label: skill })) : [];
          setValue('skills', skillsArray);
        } else if (key === 'location') {
          setValue('location', locationOptions.find(opt => opt.value === data.location));
        } else if (key === 'status') {
          setValue('status', statusOptions.find(opt => opt.value === data.status));
        } else if (key === 'application_deadline') {
          setValue('application_deadline', data.application_deadline ? 
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const submitData = {
        ...
        skills: data.skills?.map(skill => skill.value).join(', ') || '',
